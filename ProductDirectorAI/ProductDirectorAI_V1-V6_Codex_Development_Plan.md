...
目标工程选择是 React/TypeScript 前端、Python/FastAPI 后端、PostgreSQL 持久层、独立 Python Worker、Blender、FFmpeg、本地文件/S3 兼容存储接口。当前真实原型为 React JSX、FastAPI、SQLite 与进程内 BackgroundTasks；它可继续作为 V1 基线，但不等于目标生产栈已经完成。差异、限制与渐进迁移见 ADR 和差距文档，不得为符合文档而大规模重写可用系统。
...
“儿童拳击机 · 墨西哥西语短视频”只是可替换的演示/验收例子，不是唯一主场景或业务默认。验收应至少采用一个许可明确、具有相应几何/交互特征的产品；产品类别、国家、语言和平台都不得写死在代码、合同或默认镜头规则中。
...
用户于 2026-09-10 提供过单卡 RTX 5090 32GB、14C64GB、上海区域、50GB 系统盘、按量 ¥3.20/h 的购买前创建页候选；它不是最终采购事实，价格也仅为历史快照。V1 单节点 Blender 部署可独立验证图片/GLB 链路；ComfyUI 工作流验证属于 V2 或另行明确授权，不能作为 V1 退出门。采购与自动启停 API 属于更后续的受限自动化能力，也不得与已授权的单节点部署混为一谈。
...
1. **可替换参考场景**：同一许可明确的代表性产品（可使用儿童拳击机，但不限定该品类），1 个计划、3 个 Profile、各 2 个变体，产生 6 条真实可播放成果。至少一个目标语言版本（可为 es-MX）含实际字幕、配音和 BGM；全部有对应发布包与成本记录。
...
| F02 | 许可明确、可替换的代表性产品模型与多视图；需要接触测试时选择含明确交互锚点的产品（儿童拳击机仅为示例） | 通用业务演示、锚点接触 |
...
以下内容亦提供为独立文件 `CODEX_SOL56_START_HERE.md`。当前优先采用新电脑交接指令，不从零重建：

> 接手 ProductDirectorAI。先读 `AGENTS.md`、`docs/HANDOFF_NEW_COMPUTER.md`、文档总入口、当前状态、执行记录、V1 差距、云交接、历史验收和完整 V1–V6 规划。检查 Git 与新电脑环境，保留用户工作和现有 V1 原型，不从零重建。当前仅授权 V1；将历史证据与本次复验分开记录，完整退出门未满足前保持 PARTIAL，不进入 V2–V6。
>
...
规划交付检查：本文件涵盖六期产品目标、界面、用户流程、模块、数据、API、Provider、本地/云端、3D/Blender/ComfyUI/MiniMax、状态、测试、验收、禁止事项、施工任务与退出条件；配套文件提供起始 Schema、有效 JSON 示例、原始 UI 参考图和首条执行指令。下一步按 `docs/HANDOFF_NEW_COMPUTER.md` 在新电脑审计并复验已有 V1 原型；不从零重建，也不因历史 P0 记录自动宣布完整 V1 通过。
