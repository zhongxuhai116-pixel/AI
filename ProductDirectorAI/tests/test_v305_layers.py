...
def make_composite_fixtures(root: Path) -> dict:
    ...
    for frame in range(FRAMES):
        product = np.zeros((HEIGHT, WIDTH, 3), dtype=np.float32)
        product[py:py + ph, px:px + pw] = appearance
        save_rgb(product, dirs["product"] / f"prod_{frame:04d}.png")
...
def run_composite(out_dir: Path, fixtures: dict, *extra: str) -> tuple[int, dict]:
    result = subprocess.run(
        [sys.executable, str(COMPOSITE_SCRIPT),
         ...
         "--background", fixtures["bg"], "--out", str(out_dir), "--dilate", "2", *extra],
