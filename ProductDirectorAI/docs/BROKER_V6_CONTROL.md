...
| V4-01…03 | 锚点/人物/动作版本；Proxy、IK/时序/碰撞；接触距离≤2cm或更严格锚点半径、偏差≤2帧、穿透≤1cm且不持续超过2帧 | V4-01 合同已实现（2026-09-13）：锚点（产品本地规范坐标 + 单位法线 + 半径 + 允许动作白名单）、修订不可改写、锚点集按修订冻结（幂等 + 哈希绑定）、产品版本升级后锚点不自动沿用（测试钉住）；人物规格（合成 Proxy / 授权素材需许可+同意记录）；四个必需动作模板；纯几何 bbox→世界坐标变换。Proxy/IK/碰撞（V4-02/03）未开始。**注意：V3 阶段门仍待负责人人工复核（V3_ACCEPTANCE 草稿），V4 工程按项目先例（V2 曾在 V1 验收前开工）先行推进，不宣布 V3 PASS** |
