...
| V6-01…05（A） | 六个Profile、es-MX文案/TTS/字幕、BGM/ducking/响度、Batch、封面与发布包 | **V6-01 已完成（2026-09-13）**：Platform Profile 与后期模板合同（字段齐全、BLOCKING/WARNING 分级校验、硬限制必须带来源+核验时间、规则状态 NOT_VERIFIED/STALE/CURRENT）、六个种子 Profile（TikTok MX 9:16 es-MX / YouTube Shorts 9:16 en / IG Reels 9:16 en / Facebook Ads 素材 1:1 es-MX / Marketplace 1:1 es-MX / Pinterest 2:3 en）、版本化 API（版本号服务端强制递增、旧版本保留）、成片兼容性校验（1:1 Profile 拒绝 9:16 成片，不盲目裁切）、账号能力如实 NOT_CONFIGURED；线上冒烟 16/16，后端 409/409，见 [V6-01 报告](reports/V601_PLATFORM_PROFILE_2026-09-13.md)。**V6-02…05 未开始**：es-MX 文案/字幕/TTS 试听、BGM/ducking/响度、Batch 矩阵、封面与发布包 |
