...
## 2026-09-13 · 控制端 V6 用户流程独立复核

- 基线 `a847b46`；真实浏览器复现时长无法选择、批次旧预览不失效、切计划保留旧产品版本、窄菜单无名称与重复Profile标签。最新用户成片由ffprobe确认1080×1920/24fps/144帧/6秒，尽管提示词要求15秒5场景；实际GLB只有单一灰色材质，无图片/贴图/UV/顶点色。
- 形成[9项BUG与修改任务单](reports/V6_UI_RENDER_BUG_AUDIT_2026-09-13.md)，包含证据、根因、修复顺序与回归标准；同步当前阶段和V6验收结论为用户流程FAIL/待修复。
- 指定部署Node v22.23.2后重跑前端build与Sites 4/4通过；首次SSH默认Node v12导致构建失败，已明确正确运行时。只读核对现有实际产物、无副作用批次预览；未创建新渲染或发布任务，未修改业务代码，不把修改任务记为已修复。
...
### 2026-09-13 云端会话：V6 开工（V6-01 Platform Profile 与后期模板）

- **主规划 V6-01 完成**（按项目先例：V3/V4/V5 阶段门待负责人复核，V6 施工不改变这些门的状态）：新增合同模块 `platform_profiles.py`——`PlatformProfileSpec`（身份/市场/视频/构图/文案/字幕/配音/BGM/混音/发布默认/规则来源共 11 组字段）与 `PostproductionPresetSpec`（字幕/配音/BGM/混音 + 配音过长适配策略 + 固定后期顺序），校验器输出 BLOCKING/WARNING 分级并定位字段。**诚实边界**：平台硬限制一律留空（任何 `hard_limits` 条目必须带 `source_url` + `verified_at`，否则阻断）；规则状态如实 `NOT_VERIFIED`（>180 天为 `STALE`）；文案上限标注为本产品默认并声明计数算法；字幕字体用云端已装的 DejaVu Sans 并记录 Bitstream Vera 许可；配音默认关闭（未配置 TTS Provider）；BGM 默认关闭且商业用途 Profile 要求 `commercial_use_allowed`。**六个种子 Profile**：TikTok MX 9:16/es-MX、YouTube Shorts 9:16/en、IG Reels 9:16/en、Facebook Ads 素材 1:1/es-MX、Marketplace 1:1/es-MX、Pinterest 2:3/en（均为产品预设，明示不代表账号存在或存在官方发布 API）。**版本化 API**：`GET/POST /platform-profiles`、详情/版本列表/新版本（版本号服务端强制递增、旧版本不可改写）、`POST …/validate`（规格 + 可选真实成片兼容性 + 账号能力如实 `NOT_CONFIGURED`）、后期模板的版本化读写；新增四张表（SQLite 内联与 `db/schema.postgres.sql` 同步）并在启动时幂等写入种子。测试新增 `tests/test_v6_profiles.py` **20 例**（含比例不一致、主体越出安全区、硬限制未核验、BGM 缺许可、非商用 BGM 用于广告素材、字幕缺字体许可/SRT、规则过期、配音语言不匹配、成片比例/时长兼容等负例）；**线上真实冒烟 16/16**（1:1 Profile 正确拒绝 9:16 成片并提示按 crop_policy 处理）；冒烟抓到并修复真实缺陷：草稿非法 spec 的 ValidationError 未捕获会变成 500，现改为写库前校验并返回 422。后端全量 **409/409 OK**。证据见 [V6-01 报告](reports/V601_PLATFORM_PROFILE_2026-09-13.md)。V6 保持 IN_PROGRESS，V6-02…05 未开始。
- **主规划 V6-04 批次与变体完成**：`batch.py`（矩阵展开 产品版本 × 计划版本 × Profile 版本 × 变体、**总数由服务端给出**、上限 100、矩阵与 `items[]` **互斥**、相容性冲突整批 422 逐条列出、去重提示不自动合并、缓存键、状态聚合、只重试失败项）+ 8 个 API（preview 无副作用 / create 幂等且每项独立 Run / items 分页 / pause 只停新调度 / resume / cancel 范围可选并重算聚合 / retry-failed / archive）。**修 5 个真实缺陷**：① **批次项只写 `jobs`/`runs` 未写 `run_jobs` 队列行 → Worker 永远领不到任务（批次"不执行"的真因）**；② `runs.job_id` 外键要求先插 `jobs`（SQLite 侥幸通过、PostgreSQL 拒绝）；③ **`job_events` 的 `MAX(sequence)+1` 在并发写入下撞 PostgreSQL 唯一键 → 真实导致 15 秒广告渲染到 75% 失败**（改为有界重试重算序号 + 4 线程并发测试）；④ 取消后不重算聚合状态；⑤ 调度失败静默中断。测试 19 例 + 并发 2 例；线上真实批次（1×1×3×2、并发 1）出片 5 条已下载。见 [V6-04 报告](reports/V604_BATCH_2026-09-13.md)。
- **主规划 V6-05 发布包完成（含真实广告闭环）**：`publish_package.py`（12.7 包结构、Manifest 字段、凭证/临时链接检查、许可清单且**音乐原文件默认不入包**、无凭证发布模板、ZIP 防穿越与哈希复核、审批绑定内容哈希、批次归档只收已审批包）+ 7 个 API。**真实闭环**：宇航员星空灯 15 秒西语广告——参考图 → Hunyuan3D 单视图重建 GLB → **V6 生产计划（5 场景 360 帧 15.000s）** → 真实 Blender 多通道渲染（1080×1920）→ 5 段真实 H3 场景背景 → 逐场景 Strict 合成（5/5 passed、pixel_lock_ok）→ 程序化接地阴影与落地 → es-MX 配音（离线 14.709s）→ 混音 **−16.2 LUFS / −1.0 dBTP** → **精确场景字幕**（0–3/3–7/7–10/10–13/13–15）烧录 → 发布包 build/verify/approve → **ZIP 27MB 下载**。**如实缺口（不掩盖）**：该片产品保真未达 brief——单视图重建**无贴图**（面罩/镜片/比例与原图不一致）、产品在部分场景悬空（阴影是遮罩模糊生成的程序化阴影，非渲染层）、配音为离线预览音色、无 BGM；需官方带贴图模型 / 多视图重建 / H3 产品替换参考视频 / 商业配音与授权 BGM 之一才能达到正式投放标准。测试 15 例。见 [V6-05 报告](reports/V605_PACKAGE_2026-09-13.md)。

- **主规划 V6-06 资源与成本账本完成**：新增 `ledger.py`——用量事件按 (provider, operation_id, billing_item, event_id) 的 sha256 **去重**（重放返回已登记事件，不重复计费）、**四类金额**（estimate/reserved/accrued/settled）且 **reserved 不与 settled 相加**、`available = limit − settled − reserved_outstanding − accrued_unreserved`（服务端算，UI 不自行相加）、`check_budget` 三态阻断（无预算/无上限/额度不足）、**无已知报价不按 0**（`amount=null` + 原因 + `unpriced_event_ids` 单列）、自管资源费率标注 `internal_estimate`、`settle_reservation` 对账（超支记 `additional_accrual` 并 `overrun=true`、低于预留 `release`）、**预留行 `settled_at`/`settled_entry_id` 阻止重复结算（否则双计成本）**、预算上限 revision 乐观锁且只影响后续提交。API 9 个：`GET /rate-card`、`POST /usage-events`、`GET /usage`、`GET /costs`、`GET/POST /budgets`、`PATCH /budgets/{id}`、`POST /budgets/{id}/reserve`、`POST /budgets/{id}/settle`；新增 `budgets`/`usage_events`/`cost_ledger` 三表（内联 SQLite 与 `db/schema.postgres.sql` 同步，含 `ensure_column` + 幂等 `ALTER TABLE` 兼容既有安装）。**真实生产动作自动记账**（不是空账本）：Worker 渲染任务墙钟 GPU 秒（**QA_REJECTED 与失败任务同样计入消耗**）、H3 生成请求、ffmpeg 混音 CPU 秒、TTS 字符数；记账失败不阻断生产但打印 stderr，绝不伪造金额。前端新增「成本与用量」页（预算四类金额/可用额度、建立与按 revision 调整上限、预留、逐条对账含超支提示、用量登记、费率卡、未定价事件告警、账本明细）。测试 `tests/test_v6_ledger.py` **23 例**；后端全量 **516/516 OK**；**真机 PostgreSQL 冒烟 18/18**（含超支对账、重复结算 409、revision 冲突 409、无报价 `amount=null`）；前端 build 成功 + Sites 4/4 + web 200。如实边界：不是真实账单（内部费率）、未接外部 Provider 计费 API（无报价留空）、预估不是封顶、无多币种换算、无账本导出报表。见 [V6-06 报告](reports/V606_COST_LEDGER_2026-09-13.md)。

- 未完成（如实记录）：V3/V4/V5 负责人人工复核（三份 ACCEPTANCE 草稿）；V4 尾巴（真人感人物层、双人动作、修订流程、浏览器走查）；V5 的 10 条许可参考素材人工标注夹具；四个平台连接器的真实发布验证（无授权 → BLOCKED）；浏览器人工走查；商业配音与授权 BGM。

- **主规划 V6-16 权限/回归/手册/OpenAPI/示例/验收完成（V6 开发项收官）**：新增 `roles.py`——Owner/Editor/Reviewer/Publisher 权限矩阵与**服务端路由→权限表强制**（未登记的写操作默认拒绝；`GET` 未登记按 `project:read`），**职责分离**：Reviewer 可审批发布包（`package:approve`）但不能发布、Publisher 可发布（`publish:write`）但不能审批/改计划、Editor 可建计划与批次但不能审批发布包、Owner 全权含成员与自动化/Webhook 管理；成员令牌 `pdm_<id>_<secret>`（服务端只存哈希、明文只显示一次、撤销立即 401、记录 `last_used_at`/`call_count`），`security.authenticate` 增加成员分支，中间件对成员身份执行权限检查（越权 403 `permission_denied` 并返回所需权限）；API：`GET /roles/matrix`、`GET/POST /members`、`DELETE /members/{id}`（幂等）。`scripts/export_openapi.py` 从运行时导出 [docs/openapi/productdirector-v6.json](openapi/productdirector-v6.json)（**153 路径 / 178 操作 / 87 schema**，扩展字段含角色矩阵、Automation scope 与限额、Webhook 事件目录与投递约定、发布状态机与诚实边界）。新增 [V6 运行手册](RUNBOOK_V6.md)：服务与目录、部署升级（幂等迁移）、**Worker/调度 tick/投递 tick 的 systemd 单元模板**（产品内无常驻调度进程）、备份与恢复步骤与恢复后核对清单、密钥与凭据轮换（含平台 OAuth 环境变量）、日常巡检五项、故障处置表、回滚、保留策略（未实现，如实标注）。新增 [示例脚本](../examples/automation_client.sh)（外部客户端最小可运行流程：调用面→预检→幂等创建→重放→用量→发布面，脚本内不含任何凭据）。新增 [V6_ACCEPTANCE](reports/V6_ACCEPTANCE.md)：V6-01…16 逐项交付与证据索引、整体验证矩阵、**未完成/阻塞项**（无真实平台授权、无第三方 Webhook 接收方、无商业配音与授权 BGM、V3/V4/V5 人工复核、浏览器走查、宇航员广告保真缺口）与发布建议（不对外声称已支持一键发布）。新增测试 `tests/test_v6_roles.py` **16 例**；**真机 PostgreSQL 冒烟 16/16**（三角色令牌真实越权 403 与所需权限回显、成员列表不含令牌/哈希、撤销后 401、审计计数）；后端全量 **628/628**、前端 build + sites 4/4（控制台 19 个导航项覆盖 V6 全部能力）。**V6 开发项至此全部完成，状态 READY_FOR_RELEASE_REVIEW**：真实平台投稿需用户提供应用凭据与账号授权，V3/V4/V5 阶段门与浏览器走查需负责人完成——因此不宣布 V6 完整 PASS。

- **主规划 V6-10…15 发布框架与四个平台连接器完成**：新增 `publishing.py`（状态机显式迁移表，**DRAFT→PUBLISHED 等跳跃被拒**；预检逐条阻断包审批/哈希/QA/审批引用/可见性（**不跨平台照搬隐私取值**）/三项声明/文案/许可/时长/时区，能力未实时查询给 WARNING；审批必须引用预检 `snapshot_hash` 且显式确认四字段，任一绑定字段变化即失效；防重键 = `package_version+account+publish_intent_id`；`interpret_query` 只有上游确认才标 PUBLISHED、未知→RECONCILING、**无公开链接只留平台 ID 不捏造 URL**；`cancel_capability` 与 `retry_decision` 如实返回支持程度与"结果未知先对账"）与 `publishing_connectors.py`（TikTok / YouTube / Instagram Reels / Facebook Page：官方端点与 scope 来自 `docs/integrations/*.md`；**官方未确认的限制不写死**，作为 `unconfirmed_limits` 只给 WARNING；缺凭据 → `NOT_CONFIGURED` 并列出所需环境变量；**未配置不生成假授权链接**；`exchange_callback`/`refresh_authorization` 是真实 HTTP 流程（测试用本机 stub provider 走通换 token，校验 `client_key`/`code_verifier`，令牌只存引用、明文不落库不回传），刷新失败返回 `AUTH_REQUIRED` 不循环重试；`get_account_capabilities` 按平台走 creator_info/channels.list/Graph；`submit_publish` 在本部署**不发起真实上传**并说明原因，TikTok 缺 creator_info 先报前置错误；不支持的平台取消显式 `cancel_unsupported`；保留平台人工交互、禁止浏览器自动化绕过）。API 11 个（`/publishing/connectors|accounts|accounts/connect|oauth/{platform}/callback|accounts/{id}(DELETE)|preflight|approvals|jobs|jobs/{id}|reconcile|retry|cancel`），新增五张表（`connected_accounts`/`oauth_states`/`publish_preflights`/`publish_approvals`/`publish_jobs`，内联 SQLite 与 `db/schema.postgres.sql` 同步），Automation scope 读 `packages:read`、写 `publish:write`，`POST /publishing/jobs` 进入强制幂等表；前端新增「发布与审批」页（连接器状态与官方资料、授权入口、账号断开、预检表单、审批快照、任务对账/取消与四条诚实性说明）。测试 `tests/test_v6_publishing.py` **30 例**；**真机 PostgreSQL 冒烟 16/16**（四个连接器全 BLOCKED、无假链接、state 一次性与未知 state 400、无账号预检阻断且不产快照、不可执行预检不许审批、scope 越权 403、真实部署无假账号/假任务）；后端全量 **612/612**、前端 build + sites 4/4。**如实缺口（不掩盖）**：没有任何真实平台发布——无应用凭据与账号授权，真实上传/平台处理/公开链接未验证；`submit_publish` 有意不启用真实调用；OAuth 仅在本机 stub provider 上验证；`automation_policy` 审批只有范围与失效规则、没有执行器；没有平台侧回调入口；Pinterest/Amazon/Marketplace 仍为人工导入路线。见 [V6-10…15 报告](reports/V610_PUBLISHING_CONNECTORS_2026-09-13.md)。

- **主规划 V6-09 真实总控台与负载/故障恢复完成**：新增 `GET /api/v1/console/overview`（单请求返回作业状态与窗口吞吐、成功率（SUCCEEDED+VERIFICATION_PASSED）、Run 状态、队列深度含 `oldest_queued_at` 与 **`jobs_without_queue_row`**、批次/项聚合、失败原因 Top5、账本四类金额与未定价用量、Outbox 积压与最近事件、投递计数与**投递延迟 p50/p95**（同库时钟）、Automation Key 计数、`measured_at` 与口径说明；前端新增「总控台」页（6 指标卡 + 5 秒自动刷新 + 15 分钟–24 小时窗口）。**真机负载实测**（`scripts/v6_load_test.py`，100 项批次 / 2 个真实 Worker 进程 / 调度与投递 tick）：批次创建 **84 ms**、`preview` p95 **48.65 ms**（门 < 2 s）、**172.5 s 完成、34.78 项/分钟、100/100 SUCCEEDED**；中途删除 **10 项**的 `run_jobs` 队列行复现产线故障 → **10/10 恢复**；干净端点事件投递专项测量（`scripts/v6_latency_probe.py`）**20/20 送达，p50 982.8 ms / p95 1815.1 ms**（门：通常 3 s 内）。**实测中发现并修复 4 个真实缺陷**：① 批次项完成后无人推进批次（100 项只调度 2 项、98 项永久 PENDING）→ Worker 完成作业即推进所属批次 + 新增调度入口 `POST /internal/v1/batches/advance`（Worker 令牌、`execute_inline=false`）；② 多调度器并发为同一项创建 Run 撞 `idx_runs_plan_id_idempotency`，且 PostgreSQL 事务中止后继续写导致 500 与整批停滞 → 原子认领（条件 UPDATE + rowcount）、**每项独立事务**、同键 Run 复用、认领超时（>300 s）自动退回 PENDING；③ 新注册端点会收到全部历史事件（数百条投递洪水，新事件延迟数十分钟）→ `webhook_endpoints.backfill_history` 默认 0，只有显式开启才回填（`ensure_column` + 幂等 `ALTER TABLE` 兼容旧库）；④ 投递队列按全局事件时间取前 N 条，历史/不可达目标占满名额使新端点 `attempted=0` → `_pending_deliveries` 改为**按端点轮询**（每端点每 tick ≤5 条、连接级失败目标 60 s 冷却、`skipped_endpoints` 透明返回）。新增回归测试 `tests/test_v6_console_load.py` **10 例**（聚合与口径、缺队列行可见、认领超时回收、同键 Run 复用、Worker 完成推进批次、新端点不回填、显式回填、注册后新事件必达、不可达端点不饿死健康端点、全局上限生效）；后端全量 **582/582**、前端 build + sites 4/4。另：四个平台连接器的官方资料调研已完成并入库（`docs/integrations/{tiktok,youtube,instagram,facebook_page}.md`，含官方端点/scope/素材限制/必须保留的用户交互/未确认项清单），供 V6-10…15 施工使用。如实边界：吞吐基于 540×960/6 s 计划（非 1080p）、单机 2 Worker、无多机与长稳测试、故障只注入队列行缺失一类、无常驻调度进程（需 systemd timer）、Outbox/投递记录无保留策略。见 [V6-09 报告](reports/V609_CONSOLE_LOAD_2026-09-13.md)。

- **主规划 V6-08 事务性 Outbox 与签名 Webhook 完成**：新增 `webhooks.py`——事件目录（`batch.started/completed`、`item.completed/failed`、`review.required`、`package.ready`、`publish.*`、`budget.blocked`）、信封（`schema_version`/稳定 `event_id`/`aggregate.revision`/`delivery.at_least_once`+按 `event_id` 去重、不承诺顺序）、**载荷含凭证或媒体内容即拒绝写入**（文本模式 + 键名结构检查，不用会误伤哈希的“长随机串”启发式）、`HMAC-SHA256(secret, X-PDA-Timestamp + "." + raw_body)` 签名（时间戳/签名/event_id/attempt 分头，容差 300s）、**轮换期双密钥**（旧密钥宽限期内仍可验签，`key_index` 指示命中）、重试 1m/5m/15m/1h/6h/24h（**首次 + 6 次重试 = 7 次尝试**后进死信）、非 408/429 的 4xx 视为配置/权限错误→死信并**暂停目标**、投递记录**脱敏**（截断 + 抹除疑似凭证，列表从不回传密钥）。**事务性 Outbox**：`emit_event(db, …)` 接收业务事务连接，事件与业务写入同事务提交（新增回滚测试证明二者一起回滚）；接入点：批次状态转变（`batch.started`/`batch.completed`）、批次项终态回写（`item.completed`/`item.failed` + Job 为 `QA_REJECTED` 时 `review.required`）、发布包审批（`package.ready`）、预算预留阻断与 Automation Key 周期预算超限（`budget.blocked`，该路径无业务写入故用独立事务并如实标注）；`publish.succeeded/failed` 仅在目录中定义、**V6-10 之前不发射**；`CANCELLED` 有意不发事件。新增四张表（内联 SQLite 与 `db/schema.postgres.sql` 同步）。API：`GET /webhooks/catalog`、`GET/POST /webhooks`、`PATCH /webhooks/{id}`（revision 乐观锁）、`POST /webhooks/{id}/test`（只投该事件）、`GET /webhooks/{id}/deliveries`（脱敏 + 死信）、`POST /webhooks/{id}/dead-letters/{event_id}/replay`（event_id 不变）、`rotate-secret`/`pause`/`resume`、内部投递 Worker `POST /internal/v1/webhooks/dispatch`（Worker 令牌，`force_due` 供演练与恢复）。前端新增「事件与 Webhook」页。测试 `tests/test_v6_webhooks.py` **25 例**（含真实本机接收端：签名复算、时间戳过期、双密钥、7 次尝试死信、4xx 暂停、429 重试、重放、脱敏、订阅过滤、同事务回滚）；**真机 PostgreSQL 冒烟 19/19 无失败**（真实接收端进程：真实 500 → 重试计划 → 7 次后死信 → 重放送达 attempt=8、HMAC 独立复算一致、轮换宽限、暂停后测试投递 409、投递记录中接收端回显的 token 被 `[redacted]`）；后端全量 **572/572**、前端 build + sites 4/4。如实边界：接收端全为本机回环（无第三方真实接入与公网 TLS 证据）、不保证顺序、投递依赖被调用（无独立常驻 Worker 进程）、长周期重试窗口未实测、Outbox/投递记录无保留策略。见 [V6-08 报告](reports/V608_WEBHOOK_OUTBOX_2026-09-13.md)。

- **主规划 V6-07 Automation API 完成**：新增 `automation.py`——Key 形态 `pda_<key_id>_<secret>`（**服务端只存 sha256 与 12 位指纹，明文只在创建响应出现一次**）、7 个 scope 与**逐条登记的路由→scope 表**（未登记路由不放行）、Key 管理仅 Owner 会话（Key 调用返回 403 `owner_session_required`）、IP 允许列表（`10.0.0.*`/`*`）、**Key+工作区双层限流**（默认 60/分钟，429 带 `Retry-After` 与 `X-RateLimit-*`，并在响应与错误中标注「本产品初始设置，不是平台事实」）、**幂等** `Idempotency-Key` 按 (owner, 调用方, 接口, 键) 隔离并绑定请求规范化哈希（同键同体重放返回首次结果 + `Idempotency-Replayed`，同键不同体 409，Automation 创建类调用缺键 428）、**Key 周期预算**（账本新增 `actor_key_id`，按 settled+accrued 汇总，超额 409）。API：`POST/GET/DELETE /automation-keys`、`GET /automation/openapi`、`POST /automation/generate`（**复用批次创建同一实现**，`auto_publish=true` 必须有 `budget_id` 且有上限）、`GET /automation/batches/{id}`、`GET /automation/jobs/{id}`（**生产状态与发布状态分开**）；`security.authenticate` 增加 Key 分支，中间件执行 scope/边界/限流/预算检查。**顺手修真实缺陷**：产线数据库查到 2 个 `QUEUED` 作业的 `run_jobs` 行为 0（项会永久卡住）→ 对账 `reconcile_batch_items` 改为幂等补建 `run_jobs` + `job_attempts`，`start_batch_item_run` 复用之；**如实说明**这 2 个是孤儿作业（其 `batch_items` 行已不存在），孤儿 QUEUED 作业清扫属 V6-09，未实现不假装已解决。前端新增「自动化接入」页（Key 列表/创建/一次性密钥复制/撤销/调用面文档）。测试 `tests/test_v6_automation.py` **30 例** + 批次补建回归 1 例；**真机 PostgreSQL 冒烟 21/21**（真 Key、真越权 403、缺键 428、同键不同体 409、限流 429+Retry-After、撤销后 401、调用面文档），且冒烟经 Automation API 创建的两个批次**各产出一个真实渲染项（SUCCEEDED）**——自动化与 UI 走同一套生产服务；后端全量 **547/547**、前端 build + sites 4/4。如实边界：幂等键目前只对 Automation 调用强制（会话路径沿用 CSRF + 请求体幂等键，V6-16 统一）、限额未压测标定（V6-09）、无真实第三方客户端接入、Key 预算不含未结预留、`webhooks:manage` 只登记路由（端点属 V6-08）。见 [V6-07 报告](reports/V607_AUTOMATION_API_2026-09-13.md)。

- **主规划 V6-03 完成**：新增 `audio_post.py`——`ebur128` 响度/真峰值实测与判定（产品默认 −16 LUFS ±1.5 LU、≤ −1 dBTP，注明非平台要求）、混音（旁白/BGM 独立轨 + ducking 旁白优先 + loudnorm + alimiter，48k 立体声）、**ducking 自动阈值**（按实测旁白电平推导：线上人声均值 −23.9 dBFS → 阈值 −29.9 dB；固定 −18 dB 在真实人声上从不触发）、**分支级 ducking 验收** `verify_ducking`（成品响度归一会抵消跨文件差异，故分别渲染 ducking 前后的音乐分支，在语音段/停顿段比较同一频段电平）、频段与窗口电平测量、`detect_speech_windows`、**时长适配三策略**（延长非接触镜头受 `extend_max_frames` 与 Profile 上限双重约束、接触镜头绝不改动；调速超范围拒绝；改文案不改视频）、**构图策略感知编码**（letterbox 补边 / 居中裁切 / `reframe_3d` 明确拒绝，杜绝拉伸）+ 真实抽帧封面。API：`GET/POST /music-assets`（**必须带 license_ref**，否则 422）、混音与下载、时长适配、`POST /video-sources`（真实 Master 登记）、`POST /runs/{id}/outputs`（成片 + 无字幕 Master + 封面 + lineage：Master sha256 / 混音 sha256 / Profile 版本与 hash / 构图策略 / 响度复测）+ 下载；音轨开关语义明确（缺省自动取最新配音、空串显式关闭）；商业用途 Profile 遇到非商用 BGM → 422 `MUSIC_NOT_COMMERCIAL`。新增 5 张表（内联 schema 与 `db/schema.postgres.sql` 同步）。测试新增 `tests/test_v6_audio.py` **22 例**；**线上真实链路 21/21**：真实 es-MX 配音 5.547s + 合成测试 BGM → 混音 **−15.5 LUFS / −1.0 dBTP**（达标）→ ducking 分支实测 **语音段压低 8.6 dB**（关闭时 0.9 dB）→ 四种音轨开关产物互不相同 → 商业闸门 422 → 适配 EXTENDED +13 帧 / 调速 REJECTED（1.109× 超 1.1 上限）/ 改文案 REQUIRES_REWRITE → 真实 Master 编码成片（烧录字幕 + 无字幕 Master + 封面，最终 MP4 复测 −15.3 LUFS）→ 9:16 Master 到 1:1 Profile 用 letterbox 不拉伸。后端全量 **452/452 OK**。真实缺陷修复：滤镜 pad 被消费两次（需 asplit）、`aloop` 巨量缓冲失败（改 demuxer 层 `-stream_loop`）、输入索引按参数个数推算错位、`sidechaincompress` 缺声道布局、固定 ducking 阈值在真实人声上不生效、跨文件比较无法验证 ducking、省略配音 ID 会误带最新配音、跨比例编码拉伸画面。证据见 [V6-03 报告](reports/V603_AUDIO_POST_2026-09-13.md)。V6 保持 IN_PROGRESS。

- **主规划 V6-02 完成**：新增 `localization.py`——本地化文案（本地确定性模板 `productdirector.local-copy.v1`，**不调用付费 API**；**事实字段与生成式宣传语分离**；四类违规宣传关键词扫描含西/英）、话题标签校验、字幕构建（SRT/VTT、UTF-8 重音字符、按词换行与行数上限）、字幕校验（倒置/重叠/越界/过短/行数行长）、**语音对齐**（ffmpeg `silencedetect` 检真实语音段逐条对齐；语音段不足时回退并说明；短句按字幕惯例补齐到最小可读时长并如实记录 `padded_short_cues`）、**配音过长显式报告**三种适配策略（延长非接触镜头/调速/改文案，不截断视频或旁白）、TTS 能力接口（本机 espeak-ng `es-419` 离线预览引擎，标注 `offline_preview`；不可用时 `NOT_CONFIGURED` + 两条合法路线）、发音词典。API：`POST /runs/{id}/localizations`、列表/详情、`PATCH /localizations/{id}`（新 revision + `downstream_invalidated`，不覆盖旧文案）、`POST /localizations/{id}/subtitles`、`POST /audio/previews`（真实合成；超长 422 不截断）、试听下载、`POST /runs/{id}/voiceovers`（授权配音上传，记录许可引用）；新增 5 张表（内联 schema 与 `db/schema.postgres.sql` 同步）。测试新增 `tests/test_v6_localization.py` **21 例**；**线上真实链路 21/21**：es-MX 文案 → 真实西语配音（1.393s / 10.016s）→ 三段真实短语拼接配音 5.547s（含真实停顿）→ **3 个语音段对齐出 3 条字幕**（第 2 条真实语音 0.754s 补齐到 0.80s）→ SRT/VTT（166/168 bytes）→ 配音过长显式报告 → 人工修订 3 个 revision 保留 → 软字幕 `mov_text language=spa` 与 libass（DejaVu Sans）烧录在真实重演成片上验证。后端全量 **430/430 OK**。真实缺陷修复：本地化接口按 `runs.owner_id` 取归属（该表无此列）→ 改经冻结合同关联；**PostgreSQL 建表口径**（线上 schema 来自 `db/schema.postgres.sql`，新表最初漏加导致 `relation "localizations" does not exist`）；字段名 `copy` 与 `BaseModel.copy` 冲突 → `copy_spec` + 输入别名；配音过长此前被静默裁剪 → 先判过长再显式报告。证据见 [V6-02 报告](reports/V602_LOCALIZATION_TTS_2026-09-13.md)。V6 保持 IN_PROGRESS。
