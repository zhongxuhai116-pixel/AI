...
审计日期：2026-09-10。依据：仓库代码、历史报告及本次云终端截图。
...
- 预演：图片 FFmpeg，GLB Blender + FFmpeg；历史本机报告有真实结果。
...
| 分镜编辑影响真实片 | UI/API 可保存镜头参数，但 Blender 相机动画、焦距固定；图像也使用固定滤镜 | 修改镜头后可观察到渲染差异；计划快照进入 Worker |
...
| 远程节点 | 无可用的云 Renderer/Worker 调度实现 | 云部署、远程存储、权限、真实任务与恢复 |
...
| Blender/GPU | 固定 EEVEE；不同版本枚举、无显示器环境需测试 | 锁版本、兼容测试；GPU 渲染独立举证 |
...
6. V2–V6 未授权，不因用户给过 API key 就自动解锁。
