...
以下是购买前截图中的候选配置，保留用于决策追溯，不代表实际已购：
...
该候选基础镜像曾被判断可用于部署 V1 API、Web、Blender 和 FFmpeg，但不能视为实际节点配置或已经安装 ProductDirectorAI/ComfyUI。实际节点 `nvidia-smi` 显示的 CUDA 12.8 是驱动能力显示，不证明 CUDA Toolkit 或 PyTorch 已安装。ComfyUI 兼容性只在 V2 或单独授权后实测。
...
| 目标 | 首选 | 原因 | 备选 |
| --- | --- | --- | --- |
| 购买前 V1 云端 Blender 与普通 ComfyUI 候选 | 优云智算 RTX 5090 32GB | 当时认为速度、显存和后续 API 自动化平衡较好 | AutoDL RTX 5090 32GB |
...
选型当日快照显示 AutoDL 参考价低于优云智算：5090 32GB 约 ¥2.78/小时，4090 24GB 约 ¥1.88/小时；优云智算截图中的 5090 为 ¥3.20/小时。该比较不代表当前报价，也不构成换平台或新采购建议。
