...
1. **V4-01 已完成**：锚点/人物/动作模板合同 + 版本化 + bbox→世界坐标变换（`interaction_geometry.py`），7 例测试（`tests/test_v4_contracts.py`）。
2. **V4-02 未开工**：锚点编辑 UI、人体 Proxy、动作模板导入（可交互预演 + 事件时间线）。
3. **V4-03 未开工**：接触/IK/动作时序、碰撞检查（接触距离 ≤2cm 或更严格锚点半径、偏差 ≤2 帧、穿透 ≤1cm 且不持续超过 2 帧）。
4. V4-04…07（人物生成/授权素材、遮挡合成与接触 QA、人物与互动界面、双人非同时动作 + V4_ACCEPTANCE）未开工。
