...
2. **V4-02/03 合同与校验已完成**：交互计划（版本化 + 锚点集/产品版本绑定 + 失效联动）、纯数学校验引擎（`interaction_validation.py`：接触 ≤min(2cm, 半径)、穿透 ≤1cm 且 ≤2 帧、时序偏差 ≤2 帧、动作白名单与人物可达能力）、数据版预演时间线，9 例测试；**渲染侧未开工**（Blender 人体 Proxy、锚点编辑 UI、动作模板导入 UI、IK/碰撞几何、单次按按钮和击打真实几何验证）。
3. **V4-04 未开工**：人物生成/授权素材路线与能力校验（实际人物层和来源记录）。
4. **V4-05 未开工**：遮挡合成、可见区域保真、接触 QA（前后遮挡和产品保真报告）。
5. **V4-06 未开工**：人物与互动界面、问题帧修订流程（完整用户流程 E2E）。
6. **V4-07 未开工**：单人验收后扩展双人非同时动作、回归、`V4_ACCEPTANCE.md`。
