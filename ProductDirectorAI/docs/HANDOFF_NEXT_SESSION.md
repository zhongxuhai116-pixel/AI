...
4. **主规划 V3-07 完整验收**：两类必需产品各至少 3 镜头/2 背景、透明反射明确边界、真机校准（当前静态口径产品约占画面 5%，取景需校准）、旧版回归、性能/存储，产出完整 `V3_ACCEPTANCE.md`。
...
3. 然后推进 **V3-06 剩余（浏览器人工走查）**与 **V3-07（真机校准：取景/产品占比、两类产品×3 镜头×2 背景、旧版回归、性能/存储，产出 V3_ACCEPTANCE.md）**；Logo / 材质类故障必须**用 CC0 相机素材验证**——官方 STEP 转出的 GLB 没有材质，验不了 Logo 缺失。
