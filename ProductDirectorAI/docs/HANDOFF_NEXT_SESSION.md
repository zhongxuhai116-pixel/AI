...
| **V2** | **ACCEPTED**（同日确认） |
| **V3** | **进行中**：V3-01/02 通过；V3-03/04 完成；**V3-05 独立层真实闭环 PASS**（真实 H3 背景 + 三层独立层 + 真实遮挡物 + 双口径复验 + 双产品零误报）；**主规划 V3-04 QA 闭环补强**（阈值集/审批绑定/终态降级修复 + Logo/尺寸/错位/变色 API 级故障注入）；任务书编号 V3-06/V3-07 有云端真实证据，主规划编号下的阶段门（阈值校准、审核界面、完整验收）仍未完成 |
| 后端测试 | **345/345**（云端，2026-09-13，QA 故障注入后全量） |
...
2. **主规划 V3-04 剩余**：阈值集真实数据集校准冻结（当前默认阈值集已版本化 + 防放宽校验）；QA 问题帧的 UI 展示（并入 V3-06 质检页）；ID 通道语义（无 Object Index，以产品遮罩渲染代替并如实记录）。
3. **主规划 V3-06 审核界面**：版本审核页 + 通道查看器 + 质检页（Blender 参考、Master、最终视频、热图与问题帧跳转 + 批准 E2E）未开工——API 与 QA 问题帧数据已就绪。
...
1. 先确认云防火墙放行了**当前**出口 IP，再跑一次云端测试确认环境没漂移（期望 **341/341**）。
2. 从 **V3-05 剩余**继续：遮挡物自身投影的逐帧人工比对；再补真实人物遮挡素材（`--occluder` 已支持独立遮挡物 GLB）。
