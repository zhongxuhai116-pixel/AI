...
| **V6** | **已开工**：**V6-01 Platform Profile 与后期模板完成**（六组字段的版本化合同、六个种子 Profile、BLOCKING/WARNING 校验、成片兼容性、后期模板与配音适配策略；硬限制/规则/账号能力全部如实标注未核验或未配置；线上冒烟 16/16，[V6-01 报告](reports/V601_PLATFORM_PROFILE_2026-09-13.md)）；**V6-02 本地化与字幕完成**（es-MX 文案事实/生成分离 + 违规宣传扫描、SRT/VTT 与时间轴校验、**真实语音对齐**（沉默切分，语音段不足会回退并说明）、真实离线西语配音与试听、授权配音上传、配音过长显式报告适配策略、人工修订 revision 链；libass 与软字幕在真实成片上验证；线上 21/21，[V6-02 报告](reports/V602_LOCALIZATION_TTS_2026-09-13.md)）；**V6-04 批次完成**（矩阵展开/互斥/相容性 422/去重提示/独立 Run/暂停只停调度/取消范围/只重试失败项；修 `run_jobs` 队列缺失、外键顺序、job_events 并发撞键等 5 个真实缺陷；见 [V6-04 报告](reports/V604_BATCH_2026-09-13.md)）；**V6-05 发布包完成并跑通闭环**（15 秒西语广告：参考图→重建→5 场景 360 帧真实渲染→5 段 H3 背景→逐场景合成→配音与混音 −16.2 LUFS→精确场景字幕→build/verify/approve→ZIP 27MB 下载；如实缺口：单视图重建无贴图/产品悬空/预览音色/无 BGM；见 [V6-05 报告](reports/V605_PACKAGE_2026-09-13.md)）；**V6-03 混音与编码完成**（BGM 许可闸门、旁白优先 ducking（分支级实测语音段压低 8.6 dB）、响度实测 −15.5 LUFS / −1.0 dBTP 与最终 MP4 复测、音轨开关真的改变输出、时长适配三策略（接触镜头绝不动）、成片 + 无字幕 Master + 封面 + lineage、9:16→1:1 letterbox 不拉伸；线上 21/21，[V6-03 报告](reports/V603_AUDIO_POST_2026-09-13.md)）；**V6-06 成本账本完成**（用量去重、四类金额且 reserved 不与 settled 相加、无已知报价不按 0、内部费率标注估算、预算 revision 乐观锁、预留/超支对账、预留 `settled_at` 防重复结算双计、真实渲染/H3/混音/TTS 自动记账、控制台「成本与用量」页；测试 23 例、真机 PostgreSQL 冒烟 18/18，[V6-06 报告](reports/V606_COST_LEDGER_2026-09-13.md)）；**V6-07 Automation API 完成**（受限 Key 只存哈希且明文一次、7 scope 路由表与未登记路由拒绝、Key 管理仅 Owner、IP 白名单、Key+工作区双层限流、幂等键绑定请求哈希、Key 周期预算、`/automation/generate|batches|jobs`、控制台「自动化接入」页；真机冒烟 21/21 且自动化批次真实出片；顺手修批次项队列行缺失卡死，含回归测试；[V6-07 报告](reports/V607_AUTOMATION_API_2026-09-13.md)）；**V6-08 事务性 Outbox 与签名 Webhook 完成**（事件与业务同事务写入含回滚测试、HMAC 签名 300s 容差、轮换期双密钥、7 次尝试后死信、4xx 暂停目标、脱敏投递记录、死信重放、内部投递 Worker、控制台「事件与 Webhook」页；真机冒烟 19/19；[V6-08 报告](reports/V608_WEBHOOK_OUTBOX_2026-09-13.md)）；**V6-09 总控台与负载实测完成**（`/console/overview` 全量 SQL 聚合 + 控制台页；100 项 / 2 真实 Worker / 172.5 s / 34.78 项每分钟 / 100 成功 / 10 项注入故障 10/10 恢复 / 事件 p95 1.82 s；修复批次不推进、并发调度撞唯一键、新端点历史洪水、投递队列饥饿 4 个实测缺陷；回归 10 例；[V6-09 报告](reports/V609_CONSOLE_LOAD_2026-09-13.md)）；**V6-10…15 发布框架与四个平台连接器完成**（状态机/预检/审批绑定快照哈希/防重/对账/取消支持程度；TikTok/YouTube/Instagram/Facebook Page 连接器：官方端点与 scope、未确认限制只给 WARNING、无凭据不生成假链接、state+PKCE 真实流程、**本部署不发起真实上传**；控制台「发布与审批」页；测试 30 例、真机冒烟 16/16；[V6-10…15 报告](reports/V610_PUBLISHING_CONNECTORS_2026-09-13.md)）；**V6-16 权限与上线材料完成**（Owner/Editor/Reviewer/Publisher 权限矩阵与服务端强制、成员令牌 `pdm_`、OpenAPI 153 路径/178 操作、[运行手册](RUNBOOK_V6.md)、示例脚本、[V6_ACCEPTANCE](reports/V6_ACCEPTANCE.md)；测试 16 例、真机 16/16、后端 628/628）。**V6 开发项全部完成，状态 READY_FOR_RELEASE_REVIEW**；剩余为非开发项：真实平台授权与发布验证、V3/V4/V5 人工复核、浏览器走查、商业配音与授权 BGM |
| 后端测试 | **628/628**（云端，2026-09-13，V6-16 之后全量） |
| 前端 | build ✓ + Sites **4/4**（云端，2026-09-13，19 个导航项覆盖 V6 全部能力） |
| V6 状态 | **开发项全部完成 → READY_FOR_RELEASE_REVIEW**，见 [V6_ACCEPTANCE](reports/V6_ACCEPTANCE.md)；真实平台发布与 V3/V4/V5 人工复核未完成 |
| 代码 | GitHub `main` = V5-06 提交，云端工作区干净 |
| 云端 | api / web / postgresql / comfy-h3 四个 systemd 服务均 active |

明确没有完成的：V3/V4/V5 阶段门（负责人人工复核三份 ACCEPTANCE 草稿）、V4 尾巴（真人感人物层/双人动作/修订流程/浏览器走查）、V5 的 10 条许可参考素材人工标注、四个平台连接器的真实发布验证（无授权 → BLOCKED）、浏览器人工走查、商业配音与授权 BGM。不能对外说"V3 完成"、"V5 完成"或"V6 完成（含真实发布）"。
