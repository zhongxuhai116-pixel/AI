...
## 11. 单卡产能实测（2026-09-12）

用 `scripts/capacity_probe.py` 连续提交 3 个**重建**任务（最轻的真实 H3 栈负载），全程采样 GPU 与磁盘。

### 11.1 排队与耗时

| 指标 | 结果 |
| --- | --- |
| 提交时队列深度 | 1 → 2 → 3（连投三个，确认单卡串行） |
| 任务 1 / 2 / 3 完成时刻 | 31.3s / 62.1s / 97.9s |
| **稳态间隔（相邻完成之差）** | 约 **31–36 秒/条** |
| 含排队的平均到完成时间 | 63.8 秒 |

**口径说明**：脚本输出的 `jobs_per_day = 1354` 用的是"含排队平均"（63.8 秒），会把吞吐**低估**约一半；稳态吞吐应按 31–36 秒/条计算，即约 **100–115 条/小时**、约 **2500–2700 条/天**（重建任务）。
...
| --- | --- |
| 峰值 GPU 利用率 | **100%** |
| 峰值显存 | **5,834 MiB**（24GB 卡的约 24%） |
| GPU 采样数 | 51 |
| 每条磁盘增量 | 8,983,076 字节（约 8.6 MiB，单个 GLB 产物） |

显存只用了约四分之一，说明**显存不是当前瓶颈**；瓶颈是单卡的串行执行时间。

### 11.3 不同任务类型的产能（实测）

| 任务类型 | 单条耗时 | 单卡日产能（估算） |
| --- | ---: | ---: |
| 3D 重建（本次实测） | 约 33 秒 | 约 2,500 条 |
| H3 视频生成（576×1024 / 124 帧，第 7 节实测） | 约 450 秒 | 约 190 条 |
| 产品替换视频（重建 + Blender 多视图 + H3，第 8 节实测） | 约 510 秒 | 约 170 条 |

### 11.4 仍未完成

- 未做**长时压测**：以上是短样本推断，没有连续数小时的稳定性、失败率与显存泄漏观察。
- 未记录**云端费用**：节点为按量计费，单条成本需要结合控制台账单换算，本报告不估算。
- 未验证**多任务并发**（同一张卡上并行跑两个 H3 任务）是否可行——目前 provider 依赖 ComfyUI 的串行队列。
