# V3 任务书：产品保真与重建校验

日期：2026-09-12。依据：主规划第 9 章（V3 产品保真与重建校验）。

状态：**规划件（PLANNED），尚未实施**。按规划"每期先列任务 ID 再实施"，这份文件列出 V3 的全部必需任务、依赖与证据要求，供确认后开工。

## 0. 进入条件

| 条件 | 现状 |
| --- | --- |
| V1 ACCEPTED | **待你在确认单上判定**（工程证据已齐） |
| V2 ACCEPTED | V2 主线全部完成并有真实证据；同样需要你确认 |
| 官方 3D 资产 | 已具备：官方 STEP → FreeCAD 网格化 → GLB（121,218 顶点） |
| 多视图输入 | 已具备：用户提供的五视角产品图 |
| Blender 产品渲染节点 | 已具备：`ProductBlenderRender`（正/背/45°，studio/living/bedroom 场景） |

## 1. V3 的目标（规格原文）

建立可验证的 **Strict Fidelity**：产品主体来自已批准 3D 产品版本和可追溯 Blender 渲染，AI 只处理允许区域；每条成果可查看产品来源、保护区域、合成方式和质量报告。

关键限定：Strict 保证的是"本系统遵守已批准资产的保护合同"，**不保证输入资产等同于现实实物**。对重建资产必须分开显示"几何/外观已核实范围"与"本次合成是否保持已核实资产"。

## 2. 任务清单

| ID | 任务 | 依赖 | 退出证据 |
| --- | --- | --- | --- |
| V3-01 | 多通道渲染：每帧输出 Beauty（含透明产品层）、Alpha、Depth、Normal、Product ID/Mask | 现有 Blender 渲染脚本 | 一次真实渲染产出全部通道；Depth 标注单位与坐标空间、Normal 标注坐标空间与编码；Mask 不经颜色管理误处理 |
| V3-02 | 通道校验：通道可解析、ID 稳定（不依赖导入顺序的数字 ID） | V3-01 | 同一资产两次渲染 ID 一致；若用 Cryptomatte 则验证对象映射可提取 |
| V3-03 | FidelityPolicy 合同：mode（STRICT/CONTROLLED/CREATIVE）、保护区域、允许操作，带版本 | — | `POST /product-versions/{id}/fidelity-policies` 返回策略版本；非法与越权拒绝 |
| V3-04 | 审核与约束绑定：产品版本扩展 `verified_dimensions` / `view_coverage` / `unverified_regions` / `logo_regions` / `camera_visibility_constraints` | 现有版本模型 | `POST /product-versions/{id}/reviews` 冻结审核记录；单图重建的未观测面标 `unverified` 并限制镜头 |
| V3-05 | Strict 合成：`C = product_alpha × trusted_product + (1 − product_alpha) × generated_background`，线性空间；背景掩码向外扩张保护产品边缘；阴影/反射/人物遮挡作为独立层 | V3-01/03 | 合成产物与手工核对一致；**禁止用完整生成视频重绘可见产品主体** |
| V3-06 | 双产品检测：背景出现多余产品影像必须被检出并拒绝 | V3-05 | 构造含双产品的样例被阻断并给出热图与问题帧 |
| V3-07 | Strict QA：Logo 缺失、轮廓异常、尺寸变化、Mask/ID 错误、缺帧均须被发现并阻断 | V3-05 | 每类故障至少有一个人工构造的负例被阻断，并记录问题帧与 Shot/frame 定位 |
| V3-08 | 版本失效联动：产品版本变化使旧计划的保真审批失效，新 Run 必须绑定新版 | V3-03/04 | 改版本后旧批准失效；新 Run 绑定新版 |
| V3-09 | 审核界面：版本审核页、通道查看器（Beauty/Alpha/Depth/Normal/ID）、质检页（Blender 参考、Master、最终视频、热图与问题帧跳转） | V3-03/07 | 真实浏览器走查；审核按钮注明批准的是哪个 Manifest hash |
| V3-10 | V3 验收报告：多通道、保护区域、问题帧、审批绑定逐项结清 | V3-01…09 | `V3_ACCEPTANCE.md`，含真实产物与负例 |

## 3. 明确不做（本期不扩大能力）

- CONTROLLED / CREATIVE 不做：沿用 V2 的预览标识与显式入口，不扩大模型能力。
- 不自动修复模型缺陷：审核不通过时允许返回外部建模工具修复后重新导入新版本。
- 不用生成图把未核实区域变成已核实事实：单图重建的未观测面必须保持 `unverified`。

## 4. 已知风险与不确定点

| 风险 | 说明 |
| --- | --- |
| 通道导出工程量 | 现有渲染脚本只出 PNG 帧；多通道需要配置多个 pass 并分别落盘，还要保证编码与色彩管理不串味 |
| 合成数学 | 必须在线性空间做，显示变换要记入 Manifest；这是"看起来差不多但不可复现"的典型坑 |
| 官方模型无材质 | 官方 STEP 转出的 GLB 为 0 材质。Strict 合成可验证几何/轮廓/尺寸，但 **Logo 保护需要带纹理或材质的模型** |
| 单卡产能 | V2 实测产品替换视频约 510 秒/条；多通道渲染会增加耗时，需重新实测 |

## 5. 需要决定的三件事

1. V1 / V2 是否 ACCEPTED（决定能否按规划顺序进入 V3）。
2. 是否提供**带材质的模型或纹理**：V3-07 要求"Logo 缺失能被检出"，而当前官方 GLB 无材质，Logo 保护无法用现有资产验证。
3. V3 是否按上表开工，或先做前几项（例如先打通 V3-01/02 多通道，再决定合成与 QA 的深度）。

---

## 6. V3-01/02 实施进展与**未解决阻塞**（2026-09-12）

状态：**IN_PROGRESS / 未通过**。已提交 `--passes` 开关与校验器，但**通道文件尚未真正写出**，因此 V3-01 不能记为完成。

### 已确认可用

- `--passes` 在渲染时启用多通道配置；**主帧产物仍正常**（72 帧全部写出），说明开关没有破坏既有渲染路径；
- 关闭该开关时行为与历史完全一致（记录过的验收哈希不受影响）。

### 遇到的真实阻塞：Blender 5.2 合成器 API 与 4.x 完全不同

| 项目 | Blender 4 | Blender 5.2 实测 |
| --- | --- | --- |
| 节点树入口 | `scene.node_tree` | **`scene.compositing_node_group`**（`node_tree` 已不存在） |
| 合成输出节点 | `CompositorNodeComposite` | **该类型不存在** |
| File Output 路径 | `base_path` + `file_slots[0].path` | **`directory` + `file_name`** |
| File Output 格式 | PNG / OPEN_EXR 任选 | **只接受 `OPEN_EXR_MULTILAYER`** |

按上述差异改成"每帧一个多层 EXR"后：渲染成功（72 主帧）、日志显示 `layout=multilayer-exr`，但 `passes/` 目录**没有任何 .exr 落盘**，全盘搜索也未找到输出文件。说明该节点的落盘行为还需要进一步确认（文件名/扩展名、是否需要 `file_output_items` 配置、或需要显式启用节点）。

### 下一步（按顺序）

1. 解决多层 EXR 的落盘问题（或改用"分通道多次渲染"的替代方案：每个 pass 设一次 `view_layer` 并各渲染一遍，代价是渲染时间线性增加）；
2. 把 `validate_passes.py` 从"每通道目录"改为读多层 EXR 的 `render_slots`；
3. 通过 V3-02 后，再进入 V3-03（FidelityPolicy）与后续任务。

在这些未解决前，V3-03…V3-10 不应开工——它们都依赖可信通道。

### 6.1 后续更新：EXR 已能落盘（仍未通过 V3-02）

继续排查后解决了落盘问题，根因是 **Blender 5 需要显式创建 `file_output_items`**（不创建时渲染成功但一个文件都不写）。另外 socket 类型要用通用枚举（`RGBA/FLOAT/VECTOR`），不是 `Alpha` 这类名称。

当前实测（官方模型，72 帧、540×960）：

| 项 | 结果 |
| --- | --- |
| 主帧 | 72/72 ✓（多通道开关不影响既有产物） |
| 多层 EXR | **72/72 个文件**，每个约 20MB，合计 1.4GB（含真实数据） |
| 已注册通道 | `beauty, alpha, depth, normal` |
| `index` | **跳过**：Blender 5.2 的 RenderLayers 节点没有 `IndexOB` 输出（需另找对象索引通道的取法） |
| 读取校验 | `bpy.data.images.load()` 读该 EXR 得到 `has_data=False / channels=0`，说明**多层 EXR 的读取方式还需确认**（可能需要指定 layer/slot 或换用 OpenEXR 读取） |

因此 V3-01 已接近完成（4/5 通道真实落盘），但 **V3-02 仍未通过**：既缺稳定的对象索引通道，也还没有可用的读取实现。下一步仍是先把这两点解决，再进 V3-03。

---

## 7. 带材质素材来源（2026-09-12，用户要求执行者自行查找）

用户明确**不能提供**带材质/纹理的模型，要求在网上查找。已找到并落地：

| 项目 | 值 |
| --- | --- |
| 来源 | Polyhaven（公共 CC0 素材库） |
| 资产 | `Camera_01`（产品类道具：带镜头、背带、机身的相机） |
| 许可 | **CC0**（公共领域，可商用、无需署名；仍登记来源以便追溯） |
| 获取方式 | 通过 Polyhaven 公开 API 读取该资产的 gltf 1k 包清单，下载 11 个文件（2,429,996 字节，含 9 张贴图与 bin） |
| 转换 | Blender 导入 gltf → 导出自包含 GLB：**2,446,188 字节 / 2 网格 / 4 材质 / 9 图像** |
| 校验 | 用项目自己的 `inspect_glb` 验证：`has_textures: true`，可被 V1/V2 素材链路直接接受 |
| 存放 | 云节点 `/home/ubuntu/pd-cc0/Camera_01_textured.glb`（**不进 Git**，与用户资产分开） |

注意事项：

- 该资产是**第三方 CC0 素材**，在 V3 报告中必须与用户资产分开标注，不能表述为"用户产品"。
- 官方 CAD 模型（STEP 转出）仍是无材质的几何资产；材质/Logo 保真验证改用本 CC0 资产，几何/尺寸保真仍用官方模型——两者用途分开。

## 8. V3-02 两个尾巴的排查结论（2026-09-12）

### 8.1 多层 EXR 读取：路径已确认

`bpy.data.images.load()` 读多层 EXR 得到 `has_data=False / channels=0`（背景模式不可用）。改用 **OpenEXR** 库：在项目 venv 中安装成功，且**能打开该多层 EXR**（首次探测报错是我读 `channels` 属性的写法问题，不是库的问题）。V3-02 校验器将改用 OpenEXR 读取。

### 8.2 对象索引通道：Blender 5.2 不提供

启用 `use_pass_combined/z/normal/object_index` 后，RenderLayers 暴露的输出只有：

```
['Image', 'Alpha', 'Depth', 'Normal']
```

即 **Blender 5.2 合成器没有 Object Index 输出**，规格里的 "Product ID/Mask" 不能从该通道取得。替代方案（按推荐度）：

1. **产品专用遮罩渲染**：额外渲染一遍"只显示产品网格"的画面，其 Alpha 即纯净产品遮罩。语义清晰、确定性强，代价是渲染时间接近翻倍，需重新实测产能。
2. 渲染前隐藏灯光与地面（同样是额外渲染，但会改变光照语义）。
3. 用 AOV 自定义通道（需逐材质配置，改动面大）。

推荐第 1 条。在选定并实现前，V3-01 记为**未完成（4/5 通道）**，V3-03 之后不开工。

## 9. 遮罩已实现，但多层 EXR 仍只写入一层（2026-09-12）

### 9.1 已完成并有证据

| 项 | 结果 |
| --- | --- |
| 主帧 | 72/72 ✓ |
| **产品遮罩（第 5 通道）** | **72/72 PNG**；覆盖率 14.65%、二值度 **0.9953**（干净遮罩） |
| 两次渲染总耗时 | 36.3 秒（72 帧；遮罩那遍只渲产品，代价很小） |

实现方式：`render_mask_pass()` 额外渲染一遍"只显示产品网格 + 透明背景"，其 Alpha 即产品遮罩；渲染后恢复可见性、输出路径、格式与合成器状态。

### 9.2 仍未通过：多层 EXR 只含 `beauty` 一层

OpenEXR 读出的通道分组只有 `['beauty']`（RGBA），说明 **alpha / depth / normal 三个 item 没有真正写入**。已排除"边建边连导致索引错位"这个猜想（改成先建完 item 再连线，结果不变）。下一步要查的是：Blender 5 的 File Output 节点中，每个 item 各自的输入 socket 名称/归属，以及连线是否真的落在对应 item 上（`node.inputs` 与 `file_output_items` 的对应关系需要实测确认）。

### 9.3 已完全摸清的 OpenEXR 读取细节（供下一步直接用）

```python
import OpenEXR, numpy as np
part = OpenEXR.File(path).parts[0]
window = part.header.get("dataWindow")     # tuple: (array([minX,minY]), array([maxX,maxY]))
width  = int(window[1][0]) + 1             # 540
height = int(window[1][1]) + 1             # 960
names  = list(part.channels.keys())        # 当前只有 ['beauty']
plane  = np.array(part.channels[name].pixels, dtype=np.float32).reshape(height, width, 4)
```

校验器 `blender/scripts/validate_fidelity_passes.py` 已按此实现，且**不再依赖 Blender**（venv 的 Pillow 读遮罩 PNG、OpenEXR 读多层 EXR），当前唯一阻塞就是 9.2。

### 9.4 追加排查：item 与输入 socket 的对应关系已确认

实测（不渲染，仅建节点）：

```
INPUTS_BEFORE ['']                                  # 新建时只有一个空名输入
创建 item "beauty" → ['beauty', '']
创建 item "alpha"  → ['beauty', 'alpha', '']
创建 item "depth"  → ['beauty', 'alpha', 'depth', '']
创建 item "normal" → ['beauty', 'alpha', 'depth', 'normal', '']
```

即 **每个 item 会生成同名输入 socket**，因此按索引连线（0..3）本来就是正确的，之前的"索引错位"猜想可以排除。剩余怀疑方向：

1. 每个 item 是否需要额外的启用/生效标志（`file_output_items` 项上的属性）；
2. EEVEE Next 在后台渲染时是否真的把 Z / Normal 送进合成器（需要用单帧 + 逐通道单独输出做最小验证）。

下一步就用**单帧最小验证**（只渲 1 帧、逐通道分别输出 PNG）来区分这两种原因，再回到 72 帧整片验证。

---

## 10. V3-01 / V3-02 **通过**（2026-09-12）

### 10.1 根因与修正

单帧最小验证给出决定性证据：**EEVEE 确实产出 Depth 与 Normal**，而"单节点挂多个 item"时**只有第一个 item 会落盘**；改成**每个通道各用一个输出节点**后四个通道全部写出。

### 10.2 真实产物（官方 STEP 模型，540×960，72 帧）

| 通道 | 帧数 | 说明 |
| --- | ---: | --- |
| beauty | 72 | 多层 EXR（`passes/beauty/`） |
| alpha | 72 | EXR，`alpha.V` 通道 |
| depth | 72 | EXR，`depth.V`，单位米（相机空间） |
| normal | 72 | EXR，`normal.X/Y/Z`，相机空间 |
| **mask** | 72 | PNG（只渲产品网格的 Alpha），二值度 0.994–0.996 |
| 主帧 | 72 | PNG，既有链路产物不受影响 |

两次渲染合计约 36 秒（72 帧）。

### 10.3 校验结果（`validate_fidelity_passes.py`，`passed: true`）

抽样第 0 / 36 / 71 帧：

| 指标 | 帧 0 | 帧 36 | 帧 71 |
| --- | ---: | ---: | ---: |
| alpha 覆盖 | 0.5311 | 0.1465 | 0.3884 |
| **mask 覆盖** | **0.5311** | **0.1465** | **0.3884** |
| 遮罩二值度 | 0.9959 | 0.9957 | 0.9937 |
| 产品区最近深度（米） | 2.98 | 3.06 | 2.58 |
| 法线模长均值 | 0.9999 | 0.9996 | 0.9997 |

**产品遮罩与 Beauty Alpha 的覆盖率逐帧完全一致**——说明遮罩渲染与主渲染用的是同一相机与同一产品可见性，可以直接用于 Strict 合成；法线模长≈1 说明法线通道编码正确。

已知语义说明：`depth` 的最大值为 `1e10`，这是 Blender 的"无命中"哨兵值（背景像素），不是缺陷；V3-05 合成时需要按该哨兵处理背景。

### 10.4 V3 状态更新

| 任务 | 状态 |
| --- | --- |
| V3-01 多通道渲染 | **PASS**（5 通道 + 主帧，72/72） |
| V3-02 通道校验 | **PASS**（校验器已不依赖 Blender；上述证据） |
| V3-03…V3-10 | 待开工 |
